    ...
    <section id="education" className="py-20 relative bg-background">
      <div className="container mx-auto px-4">
        ...
        <div className="max-w-4xl mx-auto relative">
          {/* Timeline Line */}
          <div className="absolute top-0 bottom-0 left-6 md:left-1/2 w-1 bg-gradient-to-b from-cyan-400 via-fuchsia-400 to-yellow-400 transform -translate-x-1/2 z-10"></div>
          
          {educations.map((edu, index) => (
            <div key={edu.id} className="relative mb-16 last:mb-0">
              <div className={`flex flex-col md:flex-row items-center ${edu.id === 2 ? 'md:pr-4' : ''}`}>
                ...
                {index % 2 !== 0 ? (
                  <>
                    {/* This content is for odd indexes - 12th class is index 1 */}
                    <div className={`glassmorphism rounded-xl p-6 md:w-1/2 ${edu.id === 2 ? 'md:mr-20 md:pr-8' : 'md:mr-12 md:pr-6'} md:text-right`}>
                    ...
                    <div className={`md:w-1/2 ${edu.id === 2 ? 'md:pl-4' : 'md:pl-8'} hidden md:block`}></div>
                  </>
                ) : (
                  <>
